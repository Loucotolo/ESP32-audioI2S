...
#define AUDIO_FAIL(cmd) if(audio_fail_stream) { cmd; audio_fail_stream(chbuf); }
//---------------------------------------------------------------------------------------------------------------------
...
bool Audio::connecttohost(const char* host, bool mp3local, const char* user, const char* pwd) {    
    ...
    if(strlen(host) == 0) {
        AUDIO_FAIL("Hostaddress is empty");
        return false;
    }

    if(strlen(host) >= 512 - 10) {
        AUDIO_FAIL("Hostaddress is too long");
        return false;
    }
    ...
    if(p > 0){
        AUDIO_FAIL("Hostaddress is wrong");
...
//    strcat(resp, "Transfer-Encoding: \r\n");  // otherwise the server assumes gzip compression
    ...
    AUDIO_FAIL(sprintf(chbuf, "Request %s failed!", l_host);)
...
int Audio::read_WAV_Header(uint8_t* data, size_t len) {
    ...
    if(m_controlCounter == 2){
        ...
        if((*data  != 'W') || (*(data + 1) != 'A') || (*(data + 2) != 'V') || (*(data + 3) != 'E')) {
            AUDIO_FAIL("format tag is not WAVE");
            return -1;//false;
        }
        else {
            headerSize += 4;
    ...
    if(m_controlCounter == 5){
        ...
        if(fc != 1) {
            AUDIO_FAIL("format code is not 1 (PCM)");
            stopSong();
            return -1 ; //false;
        }
...
void Audio::processPlayListData() {
    ...
    if(m_datamode == AUDIO_PLAYLISTHEADER) {                    // Read header
        ...
        if(pos >= 0) {
            m_datamode = AUDIO_NONE;
            AUDIO_FAIL("Error 400 Bad Request");
            stopSong();
            return;
        }
        ...
        if(pos >= 0) {
            m_datamode = AUDIO_NONE;
            AUDIO_FAIL("Error 404 Not Found");
            stopSong();
            return;
        }
        ...
        if(pos >= 0) {
            m_datamode = AUDIO_NONE;
            AUDIO_FAIL("Error 404 File Not Found");
            stopSong();
            return;
        }
        ...
        if(pos >= 0) {
            m_datamode = AUDIO_NONE;
            AUDIO_FAIL("Error 404 Not Available");
            stopSong();
            return;
        }
        ...
        if(pos >= 0) {
            m_datamode = AUDIO_NONE;
            AUDIO_FAIL("Error 401 Unauthorized");
            stopSong();
            return;
        }
        ...
        if(pos >= 0) {
            m_datamode = AUDIO_NONE;
            AUDIO_FAIL("Error 403 Forbidden");
            stopSong();
            return;
        }
        ...
        if(startsWith(pl, "icy-")){                         // icy-data in playlist? that can not be
            m_datamode = AUDIO_HEADER;
            AUDIO_FAIL("playlist is not valid, switch to AUDIO_HEADER");
            return;
        }
        ...
        if(m_playlistFormat == FORMAT_PLS) {
            ...
            if(!f_begin){
                ...
                if(strcmp(pl, "[playlist]") == 0){          // first entry in valid pls
                    ...
                    AUDIO_FAIL("pls is not valid, switch to AUDIO_HEADER");
                    return;
                }
            }
        ...
        if(m_playlistFormat == FORMAT_M3U8) {
            ...
            if(!f_begin){
                ...
                if(strcmp(pl, "#EXTM3U") == 0){                         // what we expected
                    ...
                    m_playlistFormat = FORMAT_NONE;
                    AUDIO_FAIL("m3u8 is not valid, switch to AUDIO_HEADER");
                    return;
                }
            }
...
void Audio::processWebStream() {
    ...
    if(f_stream && !availableBytes){
        ...
        if(loopCnt > 200000) {              // wait several seconds
            loopCnt = 0;
            AUDIO_FAIL("Stream lost -> try new connection");
            connecttohost(m_lastHost);
        }
    }
...
//        if(idx > 0) chbuf[idx] = 0;
    ...
    if(!pos){
        ...
        f_icydescription = false;
        f_icyurl = false;
        AUDIO_FAIL("can't see content in audioHeaderData");
        return;
    }
    ...
    if(indexOf(hl, "ICY 401", 0) >= 0) {
        m_f_running = false;
        stopSong();
        AUDIO_FAIL("ICY 401 Service Unavailable");
        return;
    }
    ...
    if(indexOf(hl, "content-type:", 0) >= 0) {
        ...
        AUDIO_FAIL("authentification failed, wrong credentials?");
        m_f_running = false;
        stopSong();
    }
    else {
...
bool Audio::parseContentType(const char* ct) {
    ...
    if(indexOf(ct, "audio", 0) >= 0) {        // Is ct audio?
        ...
        if(indexOf(ct, "mpeg", 13) >= 0) {
            ...
            m_f_running = false;
            AUDIO_FAIL(sprintf(chbuf, "%s, unsupported audio format", ct);)
        }
    }
