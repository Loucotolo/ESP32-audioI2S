...
void Audio::setDefaults() {
  ...
  if (!m_f_m3u8data) if (m_playlistBuff) {
      free(m_playlistBuff);  // free if not m3u8
      m_playlistBuff = NULL;
    }
  client.stop();
...
//---------------------------------------------------------------------------------------------------------------------
bool Audio::connecttohost(const char* host, bool mp3local, const char* user, const char* pwd) {
  ...
  const uint32_t TIMEOUT_MS_SSL{5000};
  ...
  if (audio_fail_stream) audio_fail_stream(chbuf);
...
void Audio::processPlayListData() {
  ...
  if (m_datamode == AUDIO_PLAYLISTHEADER) {                   // Read header
    ...
    if (pos >= 0) {
      m_datamode = AUDIO_NONE;
      if (audio_fail_stream) audio_fail_stream("Error 400 Bad Request");
      stopSong();
      return;
    }
    ...
    if (pos >= 0) {
      m_datamode = AUDIO_NONE;
      if (audio_fail_stream) audio_fail_stream("Error 404 Not Found");
      stopSong();
      return;
    }
    ...
    if (pos >= 0) {
      m_datamode = AUDIO_NONE;
      if (audio_fail_stream) audio_fail_stream("Error 404 File Not Found");
      stopSong();
      return;
    }
    ...
    if (pos >= 0) {
      m_datamode = AUDIO_NONE;
      if (audio_fail_stream)  audio_fail_stream("Error 404 Not Available");
      stopSong();
      return;
    }
    ...
    if (pos >= 0) {
      ...
      if (audio_fail_stream)  audio_fail_stream("Error 401 Unauthorized");
      stopSong();
      return;
    }
    ...
    if (pos >= 0) {
      ...
      if (audio_fail_stream)  audio_fail_stream("Error 403 Forbidden");
      stopSong();
      return;
    }
    ...
    if (startsWith(pl, "icy-")) {                       // icy-data in playlist? that can not be
      m_datamode = AUDIO_HEADER;
      if (audio_fail_stream)  audio_fail_stream("playlist is not valid, switch to AUDIO_HEADER");
  ...
  if (m_datamode == AUDIO_PLAYLISTDATA) {                 // Read next byte of .m3u file data
    ...
    if (m_f_Log) {
      AUDIO_INFO(sprintf(chbuf, "PlaylistFormat: %i", m_playlistFormat);)
    }   // Show playlistdata

    pos = indexOf(pl, "<!DOCTYPE", 0);                  // webpage found
    if (pos >= 0) {
      m_datamode = AUDIO_NONE;
      if (audio_fail_stream)  audio_fail_stream("Not Found");
      stopSong();
      return;
    }
    ...
    if (m_playlistFormat == FORMAT_PLS) {
      ...
      if (!f_begin) {
        ...
        if (strcmp(pl, "[playlist]") == 0) {        // first entry in valid pls
          ...
          if (audio_fail_stream)  audio_fail_stream("pls is not valid, switch to AUDIO_HEADER");
          return;
        }
      }
    ...
    if (m_playlistFormat == FORMAT_ASX) { // Advanced Stream Redirector
      ...
      if (indexOf(pl, "<Entry>", 0) >= 0) f_entry = true;     // found entry tag (returns -1 if not found)
      if (indexOf(pl, "<entry clientskip=\"yes\">", 0) >= 0) f_entry = true;     // found entry tag (returns -1 if not found)
      if (indexOf(pl, "<entry ClientSkip =\"yes\">", 0) >= 0) f_entry = true;     // found entry tag (returns -1 if not found)
      if (indexOf(pl, "<entry clientskip = \"no\">", 0) >= 0) f_entry = true;     // found entry tag (returns -1 if not found)
      if (f_entry) {
    ...
    if (m_playlistFormat == FORMAT_M3U8) {
      ...
      if (!f_begin) {
        ...
        if (strcmp(pl, "#EXTM3U") == 0) {                       // what we expected
          ...
          m_playlistFormat = FORMAT_NONE;
          if (audio_fail_stream)  audio_fail_stream("m3u8 is not valid, switch to AUDIO_HEADER");
          return;
        }
      }
      ...
      if (startsWith(pl, "#EXT-X-STREAM-INF:")) {
        ...
        if (pos < 0) { // not found
          ...
          sprintf(chbuf, "codec %s in m3u8 playlist not supportet", pl);
          if (audio_fail_stream)  audio_fail_stream(chbuf);
          stopSong();
          return;
        }
        ...
        if (plsEntry == 0) {
          ...
          if (m_m3u8codec == CODEC_M4A) {
            ...
            if (p1 < 0 || p2 < 0) {
              ...
              if (audio_fail_stream)  audio_fail_stream("sequenceNumber not found");
              stopSong();
              return;
            }
  ...
  if (m_f_webfile && byteCounter == m_contentlength) {
    if (InBuff.bufferFilled() < 10000) {
      if (m_datamode == AUDIO_DATA && m_f_m3u8data) {
        processM3U8entries();  // we need the next audioChunk
        return;
      }
    }
  ...
  if (true) { // statement has no effect
    ...
    if (localmp3 == true) {
      if (!f_stream) {
        ...
        if (audio_start_stream) audio_start_stream("stream ready");
      }
    } else {
      if (InBuff.bufferFilled() > (maxFrameSize * prebuffer) && !f_stream) { // waiting for buffer filled
        f_stream = true;  // ready to play the audio data
        ...
        if (!f_stream) {
          if (audio_buffering_stream) audio_buffering_stream("Bufferring");
        }
      }
      if (!f_stream) return;
  ...
  if (m_f_webfile && m_controlCounter != 100 && !m_f_m3u8data) { // m3u8call, audiochunk has no header
    ...
    if (m_codec == CODEC_MP3) {
      ...
      if (res >= 0) bytesDecoded = res;
        ...
        m_controlCounter = 100; // error, skip header
      }
    }
...
void Audio::compute_audioCurrentTime(int bd) {
  ...
  if (m_codec == CODEC_MP3) {
    setBitrate(MP3GetBitrate()) ; // if not CBR, bitrate can be changed
  }
  if (m_codec == CODEC_M4A) {
    setBitrate(AACGetBitrate()) ; // if not CBR, bitrate can be changed
  }
  if (m_codec == CODEC_AAC) {
    setBitrate(AACGetBitrate()) ; // if not CBR, bitrate can be changed
  }
  if (m_codec == CODEC_FLAC) {
    setBitrate(FLACGetBitRate()); // if not CBR, bitrate can be changed
  }
